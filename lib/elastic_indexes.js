...
// indexes = ['offerings', 'products', 'catalogs', 'inventory', 'orders'];
...
var saveIndex = function saveIndex(path, batchdata, opt) {
    ...
    console.log("saveIndex path: " + path);
    console.log("saveIndex batchdata: " + JSON.stringify(batchdata));
    console.log("saveIndex opt: " + JSON.stringify(opt));
    ...
    console.log("saveIndex batchdata with a vengeance: " + JSON.stringify(batchdata));
...
var search = function search(path, originalQuery) {
	...
	console.log("current query: " + JSON.stringify(originalQuery.query[q]));
	...
	Object.keys(key).forEach(k => {
		...
		queryFields.push(k);
		stringQuery = stringQuery.concat(JSON.stringify(key[k][0]));
	    } else if (queryFields.includes(k)) {
		stringQuery = stringQuery.concat(" OR " + key[k][0]);
	    } else {
		queryFields.push(k); 
		stringQuery = stringQuery.concat(" AND " + key[k][0]);
	    }
	});
    });
    ...
    console.log("request query: " + JSON.stringify(req));
    var response = Promise.resolve(elasticClient.search(req)).then(r => { return convertElasticSearch(r); });

    // console.log("fuck my life: " + response);
    // return convertElasticSearch(elasticClient.search(req));
...
var convertElasticSearch = function convertElasticSearch(req) {
    ...
    console.log("converted full req: " + JSON.stringify(convertedFullReq));
...
var indexCreateData = function indexCreateData(keys, data, newData) {
    ...
    console.log('indexCreateData: ' + JSON.stringify(newData));
    return newData;
};
...
exports.genericCreateQuery = function genericCreateQuery(extras, name, f, req) {
    ...
    console.log("genericCreateQuery: " + JSON.stringify(q));
    // q.query.forEach(x => console.log(x));
    return q;
};

...
exports.init = function () {
    ...
    return buildIndex(indexes).resolve();
};
...
var buildIndex = function (indexes) {
...
var searchElastic = function searchElastic(path, originalQuery) {
	...
	console.log("key: " + q);
        console.log("key content: " + JSON.stringify(key));
    ...
    console.log("query text (with quotes?): " + stringQuery);
...
var convertElasticSearch = function convertElasticSearch(req) {
    ...
    console.log("input req: " + JSON.stringify(req));
...
exports.getMiddleware = function getMiddleware(reg, createOfferF, queryF, req) {
    ...
    console.log("QUERY FUNCTION: " + queryF.toString());
    ...
    if (req.method == "GET" && reg.test(req.apiUrl) && (typeof req.query.id === "undefined")) {
        ...
        if ("action" in req.query && req.query.action == 'count') {
            isCount = true;
            console.log("is this count?: " + isCount);
            handler = (count) => {
                req.apiUrl = '/' + config.endpoints.management.path + '/count/' + count;
            };
            logger.info("REQ NEW API URL: " + req.apiUrl);
        } else {
            isCount = false;
            console.log("is this count?: " + isCount);
            handler = (results) => {
                ...
                console.log("REQ NEW API URL: " + newUrl);
                req.apiUrl = newUrl;
            };
        }

...
// exports.elasticHost = process.env.BAE_LP_INDEXES_ELASTICHOST || config.indexes.elasticHost;
...
function testElasticConnection () {
    ...
    return elasticClient.ping({ requestTimeout: 3000 });
	// .then(() => {
    // 	console.log('Connected to elasticSearch server (' + exports.elasticHost + ')');
    // }, function (e) {
    // 	console.trace(e);
    // });
...
exports.init = async function () {
    ...
    elasticClient.ping({ requestTimeout: 3000 }).then(function () {
	logger.info("Connected to elasticSearch! (" + exports.elasticHost + ")");
	buildIndexes(Object.values(exports.elasticIndexes)).then(function (indexes) {
            return indexes;
	}, function (err) {
            throw new Error(err);
	});

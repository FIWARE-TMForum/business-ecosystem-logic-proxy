...
const searchEngine = (() => {
    ...
    const parseSort = (sort) => {
        if (sort === 'name') {
            return 'productOfferingName,asc'
        }

        if (sort === 'lastUpdate') {
            return 'productOfferingLastUpdate,asc'
        }

        if (sort === '-lastUpdate') {
            return 'productOfferingLastUpdate,desc'
        }

        return null
    }

    const addPaging = (queryParams, page) => {
        page = page || {}

        if (page.offset != null && page.pageSize != null) {
            // Calculate the page
            let pageN = Math.floor(parseInt(page.offset) / parseInt(page.pageSize))
            queryParams.set('page', pageN)
            queryParams.set('size', page.pageSize)
        }
    }

    const addQueryString = (url, queryParams) => {
    ...
    const search = async (keyword, categories, page) => {
        ...
        const sort = parseSort(page.sort)

        if (sort != null) {
            queryParams.set('sort', sort)
        }

        addPaging(queryParams, page)
    ...
    const searchCatalog = async (keyword, page) => {
        let url = `${config.searchUrl}/api/SearchCatalog/${keyword}`
        const queryParams = new URLSearchParams()

...
exports.proxyRequest = function(protocol, options, data, proxiedRes, postAction) {
    ...
    // Define the callback
    var callback = function(err, result, extraHdrs) {
        ...
        for (var hdr in extraHdrs) {
            headers[hdr] = extraHdrs[hdr];
        }
    ...
    if (postAction) {
        effectiveCallback = function(err, result) {
            if (!err) {
                postAction(result, function(extraHdrs) {
                    callback(err, result, extraHdrs)
                });
            } else {
                callback(err, result);
            }

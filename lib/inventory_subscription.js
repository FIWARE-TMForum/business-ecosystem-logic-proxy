...
var getHubs = function getHubs() {
    ...
    var url = createUrl("DSProductInventory", "/DSProductInventory/api/productInventory/v2/hub");
...
var deleteInventory = function deleteInventory(data) {
    return indexes.removeIndex(indexes.siTables.inventory, data.id);
};
...
exports.createSubscription = function createSubscription(path) {
    var callbackUrl = (config.https.enabled ? "https" : "http") + "://" + 'localhost' + ":" + PORT + path;
    ...
    return getHubs()
        .then(hubs => {
            if (hubs.filter(x => x.callback === callbackUrl).length > 0) {
                ...
                var req = {
                    method: "POST",
                    url: createUrl("DSProductInventory", "/DSProductInventory/api/productInventory/v2/hub"),
                    headers: headers,

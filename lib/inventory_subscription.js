...
const createUrl = function createUrl() {
    ...
    return url.format({
        ...
        pathname: '/' + inventoryConfig.path + '/hub'
    });
};

const genericRequest = function genericRequest(url) {
...
exports.createProduct = function createProduct(productItem) {
    const product = productItem.product

    product.status = 'suspended'
    product.productOffering = productItem.productOffering

    let inventoryConfig = config.endpoints.inventory;
    const inventoryUrl = url.format({
        protocol: inventoryConfig.appSsl ? 'https' : 'http',
        hostname: inventoryConfig.host,
        port: inventoryConfig.port,
        pathname: '/product'

...
const buildFederationContext = async function(req) {
    const context = {
        tmforumEndpoint: '',
        remotePartyByKey: {}
    }
...
const isOrganizationRelatedParty = function(relatedParty) {
    ...
    return typeof relatedParty.id === 'string' && relatedParty.id.includes('organization')
}

const findPartyInFederatedEndpoint = async function(federationContext, credentialId, referredType) {
    if (!federationContext.tmforumEndpoint || !credentialId) {
        return null
    }

    const cacheKey = `${referredType}:${credentialId}`
    ...
    let partyResponse
    if (referredType === 'Organization') {
        partyResponse = await partyClient.getOrganizationsByQueryInApi(federationContext.tmforumEndpoint, query)
    } else {
        partyResponse = await partyClient.getIndividualsByQueryInApi(federationContext.tmforumEndpoint, query)
    }

    const foundParty = partyResponse && Array.isArray(partyResponse.body) && partyResponse.body.length > 0 ? partyResponse.body[0] : null
...
const normalizeRelatedParty = async function(currentUser, relatedParty, federationContext) {
    console.log('*************************** Normalizing related party ***************************')
    ...
    // The related party misses the credential ID
    if (relatedParty.id === currentUser.partyId) {
        console.log(currentUser)
        name = currentUser.id
    } else {
        // Get need to download the party
        try {
            const partyResp = await getMethod(relatedParty.id)
            const party = partyResp.body
            if (party && party.externalReference && party.externalReference.length > 0) {
                const extRef = party.externalReference.find((ref) => {
                    return ref.externalReferenceType.toLowerCase() === 'idm_id'
                })
                name = extRef ? extRef.name : ''
            }
        } catch (_) {
    ...
    if (federationContext.tmforumEndpoint && name) {
        try {
            const remoteParty = await findPartyInFederatedEndpoint(federationContext, name, referredType)
            if (remoteParty && remoteParty.id) {
                partyId = remoteParty.id
                partyHref = remoteParty.href ? remoteParty.href : remoteParty.id
            }
        } catch (_) {
            // If the remote endpoint cannot resolve the party, fallback to local IDs
        }
    }
...
const getFederatedCurrentOrganizationOperator = async function(req, federationContext, role) {
    return normalizeRelatedParty(req.user, {
        id: req.user.partyId,
...
const attachSellerOperator = async function(req, body, federationContext) {
    if (federationContext.tmforumEndpoint) {
        body.relatedParty.push(await getFederatedCurrentOrganizationOperator(req, federationContext, config.roles.sellerOperator))
        return
    }
...
const attachBuyerOperator = async function(req, body, federationContext, sellerOperatorSource) {
    if (federationContext.tmforumEndpoint) {
        const source = sellerOperatorSource || {
            id: req.user.partyId,
            href: req.user.partyId,
...
const attachProductOrderParty = async function(req, body, federationContext) {
    ...
    const sellerOperator = sourceRelatedParty.find((partyRef) => {
        return partyRef.role.toLowerCase() === config.roles.sellerOperator.toLowerCase()
    })
    ...
    if (buyer) {
        ...
        await attachBuyerOperator(req, body, federationContext, sellerOperator || seller)
    }
}
...
exports.attachRelatedParty = async function(req, api) {
    ...
    const federationContext = await buildFederationContext(req)

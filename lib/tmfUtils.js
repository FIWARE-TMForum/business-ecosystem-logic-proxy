    ...
    if (req.query['relatedParty.href']) {
        ...
        if (req.query['relatedParty.id'] === req.user.id) {
            ...
            callback({
                status: 403,
                message:
                    'You are not authorized to retrieve the orderings made by the user ' + req.query['relatedParty.id']
            });
        }
    } else {
...
exports.hasPartyRole = function(req, partyList, roleName) {
    ...
    return (
        ...
        partyList.some(function(party) {
            ...
            var expectedHref = normalize(self.getIndividualURL(req, user));

            return (
                party.id === user &&
                normalize(party.href) === expectedHref &&
...
exports.getIndividualURL = function(req, user) {
    ...
    var path = (req.user && req.user.userNickname && req.user.userNickname !== req.user.id) ? 'organization/' : 'individual/';
    ...
    return utils.getAPIURL(
        ...
        hostname,
        port,
        '/' + config.endpoints.party.path + '/api/partyManagement/v2/' + path + (user || '')
    );
};

...
const operatorClient = require('./operator').operator
...
exports.isOwner = function(req, resource) {
    return 'relatedParty' in resource && this.hasPartyRole(req, resource.relatedParty, config.roles.seller);
};
...
exports.isOrderingCustomer = function(userInfo, resourceInfo) {
    ...
    for (var i = 0; i < resourceInfo.relatedParty.length && !isCust; i++) {
        ...
        if (party.role.toLowerCase() === config.roles.customer.toLowerCase()) {
            custIncluded = true;
...
exports.validateCharacteristics = function(chars) {
    const dict= {}
    for (const char of chars){
        if (dict[char.name] === 1){
            return false
        }
        const charValues = char.productSpecCharacteristicValue
        if (charValues && charValues.length === 1 && 'valueFrom' in charValues[0] && 'valueTo' in charValues[0] && charValues[0].valueFrom >= charValues[0].valueTo){
            return false
        }else if(charValues && charValues.length > 1 && 'valueFrom' in charValues[0] && 'valueTo' in charValues[0]){
            return false
        }
        dict[char.name] = 1
    }
...
const getRoleConstraint = function(roleName) {
...
const matchesRoleConstraint = function(party, roleConstraint) {
    if (!roleConstraint) {
        return true
    }

    return !!party && typeof party.role === 'string' && party.role.toLowerCase() === roleConstraint
}

const isFederatedOrganizationIdentityCandidate = function(req, party) {
    if (
        !config.federationEnabled ||
        !req ||
        !req.user ||
        !isOrganizationPartyId(req.user.partyId) ||
        !isOrganizationPartyId(party && party.id)
    ) {
        return false
    }
...
const matchesFederatedOrganizationByRemoteId = function(req, party) {
    if (!isFederatedOrganizationIdentityCandidate(req, party)) {
        return false
    }

    const userRemotePartyId = req.user && req.user.remotePartyId

    return (
        typeof userRemotePartyId === 'string' &&
        userRemotePartyId.length > 0 &&
        userRemotePartyId === party.id
    )
}

const matchesFederatedOrganizationParty = function(req, party) {
    return matchesFederatedOrganizationByRemoteId(req, party)
}

const matchesPartyIdentity = function(req, party) {
    if (!req || !req.user || !party) {
        return false
    }

    if (party.id === req.user.partyId) {
        return true
    }

    return matchesFederatedOrganizationParty(req, party)
}

exports.hasPartyRole = function(req, partyList, roleName) {
    try {
        const roleConstraint = getRoleConstraint(roleName)
        return (
            req && req.user != null &&
            partyList.some((party) => {
                return matchesPartyIdentity(req, party) && matchesRoleConstraint(party, roleConstraint)
            })
        )
    } catch (_) {
        return false
...
exports.getIndividualURL = function(req, user) {
    ...
    var path = (req.user && req.user.userId && req.user.userId !== req.user.partyId) ? 'organization/' : 'individual/';
    ...
    return utils.getAPIURL(
        ...
        hostname,
        port,
        '/' + config.tmforum.party.path + '/' + path + (user || '')
    );
};
...
exports.isValidPrice = function(price, unit) {
    ...
    const digits = currencyCodes.code(unit).digits;
    // check if price has the correct digits after the decimal point according to ISO 4217
    const priceStr = price.toString()
    const priceParts = priceStr.split('.')
    if (priceParts.length > 1 && priceParts[1].length > digits) {
        return false
    }
    return true
}

exports.isValidPercentage = function(percentage) {
...
exports.isValidDiscount = function(offerPrice) {
    if(offerPrice.percentage){
        let percentage = offerPrice.percentage;
        if (typeof percentage !== 'number') {
            if(typeof percentage !== 'string'){
                return false
            }
        ...
        if(offerPrice.price.value){
            if (typeof offerPrice.price.value !== 'number') {
                if(typeof offerPrice.price.value !== 'string'){
                    return false
                }
            }
    
            if(offerPrice.price.value < 0) {
                return false
            }
        }
    }
...
exports.isValidPhoneNumber = function(phoneNumber){
    ...
    const cleanFormat = /^\+[1-9]\d{1,14}$/ // according to E.164 format
    if (!cleanFormat.test(phoneNumber)) return false
    try {
        const checkNumber = parsePhoneNumber(phoneNumber)
        return checkNumber.isValid()
    }
    catch(error){
        return false
    }
...
const resolveFederationTmforumEndpoint = async function(req, partyId) {
    if (!config.federationEnabled || !req || !req.user || !req.user.userId) {
        return ''
    }

    const sourcePartyId = partyId || req.user.partyId
    if (!isOrganizationRelatedParty({ id: sourcePartyId })) {
        return ''
    }

    return await federation.resolveTmforumEndpointByPartyId(sourcePartyId) || ''
}

const isOrganizationRelatedParty = function(relatedParty) {
    if (!relatedParty) {
        return false
    }

    if (typeof relatedParty.id === 'string' && relatedParty.id.toLowerCase().includes('organization')) {
        return true
    }

    if (relatedParty['@referredType']) {
        return relatedParty['@referredType'].toLowerCase() === 'organization'
    }

    return false
}

const findPartyInFederatedEndpoint = async function(tmforumEndpoint, localPartyId, credentialId) {
    if (!tmforumEndpoint) {
        throw {
            status: 422,
    ...
    const foundParty = await federation.resolveFederatedOrganizationParty(
        tmforumEndpoint,
        localPartyId,
        credentialId
    )
...
const findFederatedPartyByLocalId = async function(localPartyId) {
    const remotePartyId = await federation.resolveRemotePartyIdByLocalPartyId(localPartyId)
...
const normalizeRelatedParty = async function(currentUser, relatedParty, tmforumEndpoint, isLocal = true) {
    let name = ''
    let referredType = 'Individual'
    let getMethod = partyClient.getIndividual
    let partyId = relatedParty.id
    let partyHref = relatedParty.href ? relatedParty.href : relatedParty.id

    if (isOrganizationRelatedParty(relatedParty)) {
        referredType = 'Organization'
        getMethod = partyClient.getOrganization
    }

    if (relatedParty.id === currentUser.partyId) {
        name = currentUser.id || ''
    } else {
        try {
            const partyResp = await getMethod(relatedParty.id)
            name = getIdmExternalReferenceName(partyResp.body)
        } catch (_) {}
    }
    ...
    if (!name) {
        throw {
            status: 422,
            message: `Missing idm_id external reference for ${referredType.toLowerCase()} ${relatedParty.id}`
        }
    }

    if (tmforumEndpoint && referredType === 'Organization') {
        let remoteParty
        if (!isLocal) {
            remoteParty = await findPartyInFederatedEndpoint(tmforumEndpoint, relatedParty.id, name)
        } else if (relatedParty.id === currentUser.partyId && currentUser.remotePartyId) {
            remoteParty = {
                id: currentUser.remotePartyId,
                href: currentUser.remotePartyId
            }
        } else {
            remoteParty = await findFederatedPartyByLocalId(relatedParty.id)
        }

        partyId = remoteParty.id
        partyHref = remoteParty.href ? remoteParty.href : remoteParty.id
    }
...
const attachLocalOperator = function(body, role) {
...
const attachSellerOperator = async function(req, body, tmforumEndpoint, operatorSource, isLocal = true) {
    if (tmforumEndpoint) {
        ...
        body.relatedParty.push(await normalizeRelatedParty(req.user, {
            id: source.id,
            href: source.href ? source.href : source.id,
            role: config.roles.sellerOperator,
            name: source.name,
            "@referredType": source['@referredType'] || 'Organization'
        }, tmforumEndpoint, isLocal))
        return
    }

    attachLocalOperator(body, config.roles.sellerOperator)
}

const attachBuyerOperator = async function(req, body, tmforumEndpoint, operatorSource, isLocal = true) {
    if (tmforumEndpoint) {
        const source = operatorSource || {
            id: req.user.partyId,
            href: req.user.partyId,
        ...
        body.relatedParty.push(await normalizeRelatedParty(req.user, {
            id: source.id,
            href: source.href ? source.href : source.id,
            role: config.roles.buyerOperator,
            name: source.name,
            "@referredType": source['@referredType'] || 'Organization'
        }, tmforumEndpoint, isLocal))
        return
    ...
    attachLocalOperator(body, config.roles.buyerOperator)
}


const attachPartySpec = async function(req, body, tmforumEndpoint) {
    // The seller must be included
    const seller = body.relatedParty.find((partyRef) => {
        return partyRef.role.toLowerCase() === config.roles.seller.toLowerCase()
    })

    body.relatedParty = []

    if (seller) {
        body.relatedParty.push(await normalizeRelatedParty(req.user, seller, tmforumEndpoint))
    } else { // Seller must be always included
        body.relatedParty.push(
            await normalizeRelatedParty(req.user, {
                id: req.user.partyId,
                href: req.user.partyId,
    ...
    await attachSellerOperator(req, body, tmforumEndpoint)
}


const attachOfferingParty = async function(req, body, tmforumEndpoint) {

    const schemaFunction = () => {
        body['@schemaLocation'] = config.offeringSchema;
    }
    await attachParty(req, body, schemaFunction, tmforumEndpoint)
}

const attachOfferingPriceParty = async function(req, body, tmforumEndpoint) {

    const schemaFunction = () => {
        if (body.isBundle) {
            body['@schemaLocation'] = config.partyLocation;
        }
        else {
            body['@schemaLocation'] = config.priceCompSchema;
        }
    }
    await attachParty(req, body, schemaFunction, tmforumEndpoint)
}

const attachParty = async function(req, body, schemaFunction, tmforumEndpoint) {
    // Product offering does not include a party by default
    const parties = {
        relatedParty: []
    };

    if ("relatedParty" in body && body.relatedParty.length > 0) {
        // If there are related parties, find if there is a buyer
        const buyer = body.relatedParty.find((partyRef) => {
            return partyRef.role.toLowerCase() === config.roles.customer.toLowerCase()
        });

        if (buyer) {
            parties.relatedParty.push(await normalizeRelatedParty(req.user, buyer, tmforumEndpoint))
        }
    }

    body.relatedParty = parties.relatedParty
    ...
    body.relatedParty.push(await normalizeRelatedParty(req.user, {
        ...
        role: config.roles.seller,
        name: req.user.id,
        "@referredType": req.user && req.user.partyId && req.user.partyId.includes('organization')
            ? 'Organization'
            : 'Individual'
    }, tmforumEndpoint))
...
const attachCategoryParty = async function(req, body, tmforumEndpoint) {
    // Category does not include a party by default
    // and does not include a seller, as they are owned by the operator
    body.relatedParty = []
    ...
    if (body.relatedParty.length > 0) {
        body['@schemaLocation'] = config.partyLocation;
    }
}


const attachProductOrderParty = async function(req, body, tmforumEndpoint) {
    const isLocal = false
    const sourceRelatedParty = body.relatedParty

    // Get buyer
    const buyer = sourceRelatedParty.find((partyRef) => {
        return partyRef.role.toLowerCase() === config.roles.customer.toLowerCase()
    })

    // Get seller
    const seller = sourceRelatedParty.find((partyRef) => {
        return partyRef.role.toLowerCase() === config.roles.seller.toLowerCase()
    })
    ...
    if (seller) {
        body.relatedParty.push(await normalizeRelatedParty(req.user, seller, tmforumEndpoint, isLocal))
        await attachSellerOperator(req, body, tmforumEndpoint, seller, isLocal)
    }

    if (buyer) {
        body.relatedParty.push(await normalizeRelatedParty(req.user, buyer, tmforumEndpoint, isLocal))
        await attachBuyerOperator(req, body, tmforumEndpoint, seller, isLocal)
    }
}

exports.attachRelatedParty = async function(req, api) {
    // APIs supporting the relatedParty Overide
    const apis = {
        catalog: {
            productSpecification: attachPartySpec,
            catalog: attachPartySpec,
            productOffering: attachOfferingParty,
            productOfferingPrice: attachOfferingPriceParty,
            category: attachCategoryParty
        },
        service: {
            serviceSpecification: attachPartySpec
        },
        resource: {
            resourceSpecification: attachPartySpec
        },
        ordering: {
            productOrder: attachProductOrderParty
        },
        account: {
            billingAccount: attachPartySpec
        },
        usage: {
            usageSpecification: attachPartySpec
        }
    }
    ...
    let reqPath = req.apiUrl
    if (reqPath.endsWith('/')) {
        reqPath = reqPath.slice(0, -1)
    }

    const objType = reqPath.split('/').slice(-1)[0]

    if (!(objType in apis[api])) {
        return
    }
    ...
    try {
        ...
        console.log('Error adding relatedParty')
        return
    }

    const federationSourceParty = api === 'ordering' && objType === 'productOrder' && Array.isArray(body.relatedParty)
        ? body.relatedParty.find((partyRef) => {
            return partyRef && partyRef.role && partyRef.role.toLowerCase() === config.roles.seller.toLowerCase()
        })
        : null
    const tmforumEndpoint = await resolveFederationTmforumEndpoint(
        req,
        federationSourceParty && federationSourceParty.id ? federationSourceParty.id : null
    )
    await apis[api][objType](req, body, tmforumEndpoint)

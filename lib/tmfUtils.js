...
exports.validateCharacteristics = function(chars, previousChars = null) {
    const dict= {}
    const idDict = {}
    const complianceCredentials = []
    for (const char of chars){
        if (dict[char.name] === 1 || (char.id && idDict[char.id] === 1)){
            return false
        }
        if (String(char.name || '').trim().toLowerCase() === 'compliance:vc') {
            complianceCredentials.push(char)
        }
    ...
    const previousComplianceCredentials = Array.isArray(previousChars)
        ? previousChars.filter((char) => String(char.name || '').trim().toLowerCase() === 'compliance:vc')
        : null
    if (
        previousComplianceCredentials === null
            ? complianceCredentials.length > 0
            : !equal(complianceCredentials, previousComplianceCredentials)
    ) {
        return false
    }
...
exports.hasOrganizationCountry = function(organization) {
    if (!organization || !Array.isArray(organization.partyCharacteristic)) {
        return false;
    }

    return organization.partyCharacteristic.some((characteristic) => {
        return characteristic &&
            typeof characteristic.name === 'string' &&
            characteristic.name.trim().toLowerCase() === 'country' &&
            characteristic.value != null &&
            String(characteristic.value).trim().length > 0;
    });
};
...
exports.isValidStatusTransition = function(firstSt, nextSt) {
    ...
    return firstInd > -1 && nextInd > -1 &&
        (nextInd == firstInd + 1 || nextInd == firstInd ||
            (firstInd == lifecycle.length - 2 && nextInd == firstInd - 1) ||
            (firstInd == 0 && nextInd == lifecycle.length - 1))
}
...
const assertCurrentUser = function(currentUser) {
    if (!currentUser || !currentUser.partyId || !currentUser.id) {
        throw new Error('currentUser is required to attach relatedParty')
    }
}

const normalizeRelatedParty = async function(relatedParty, currentUser) {
    ...
    if (relatedParty.id.includes('organization')) {
    ...
    return {
        id: relatedParty.id,
        href: relatedParty.href ? relatedParty.href : relatedParty.id,
        name: name,
...
const attachOperator = function(body, role) {
    const operatorId = operatorClient.getOperatorId()
    if (operatorId) {
        body.relatedParty.push({  // Operator
            id: operatorId,
            href: operatorId,
...
const attachPartySpec = async function(body, currentUser) {
    ...
    if (seller) {
        body.relatedParty.push(await normalizeRelatedParty(seller, currentUser))
    } else { // Seller must be always included
        body.relatedParty.push(
            await normalizeRelatedParty({
    ...
    attachOperator(body, config.roles.sellerOperator)
}


const attachOfferingParty = async function(body, currentUser) {

    const schemaFunction = () => {
        body['@schemaLocation'] = config.offeringSchema;
    }
    await attachParty(body, schemaFunction, currentUser)
}

const attachOfferingPriceParty = async function(body, currentUser) {
...
const attachParty = async function(body, schemaFunction, currentUser) {
    ...
    if ("relatedParty" in body && body.relatedParty.length > 0) {
        ...
        if (buyer) {
            parties.relatedParty.push(await normalizeRelatedParty(buyer, currentUser))
        }

        attachOperator(parties, config.roles.buyerOperator)
    }

    body.relatedParty = parties.relatedParty

    body.relatedParty.push(await normalizeRelatedParty({
        id: currentUser.partyId,
        href: currentUser.partyId,
        role: config.roles.seller
    }, currentUser))
...
const attachOperatorSellerParty = async function(body) {
    const operatorId = operatorClient.getOperatorId()
    if (operatorId) {
        body.relatedParty = [{
            id: operatorId,
...
const attachCategoryParty = async function(body) {
    // Category does not include a party by default
    // and system categories are owned by the operator
    await attachOperatorSellerParty(body)
...
const attachProductOrderParty = async function(body, currentUser) {
    // Get buyer
    const buyer = body.relatedParty.find((partyRef) => {
        return partyRef.role.toLowerCase() === config.roles.customer.toLowerCase()
    })
    ...
    if (buyer) {
        body.relatedParty.push(await normalizeRelatedParty(buyer, currentUser))
        attachOperator(body, config.roles.buyerOperator)
    }
}

const attachRelatedPartyObj = async function(type, entity, currentUser) {
    assertCurrentUser(currentUser)
    ...
    const types = {
        ...
        operatorCatalog: attachOperatorSellerParty,
        catalogCategory: attachOfferingParty,
    ...
    await types[type](entity, currentUser)
    return entity
}

exports.attachRelatedPartyObj = attachRelatedPartyObj

exports.attachRelatedParty = async function(req, api) {
    // APIs supporting the relatedParty Overide
    const apis = {
        catalog: ['productSpecification', 'catalog', 'productOffering', 'productOfferingPrice', 'category'],
    ...
    await attachRelatedPartyObj(objType, body, req.user)

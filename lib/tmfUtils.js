...
const isNotFoundError = function(err) {
    return err && err.status === 404
}

const normalizeFoundRelatedParty = function(relatedParty, foundParty, referredType) {
    const partyId = foundParty && foundParty.id ? foundParty.id : relatedParty.id
    const partyHref = foundParty && foundParty.href ? foundParty.href : (relatedParty.href ? relatedParty.href : partyId)
    const partyName = getIdmExternalReferenceName(foundParty) || (foundParty && foundParty.name) || relatedParty.name || ''
...
const normalizeRemoteOrganizationById = async function(relatedParty, tmforumEndpoint) {
    try {
        const partyResp = await partyClient.getOrganizationInApi(tmforumEndpoint, relatedParty.id)
        return normalizeFoundRelatedParty(relatedParty, partyResp.body, 'Organization')
    } catch (err) {
        if (isNotFoundError(err)) {
            throw {
                status: 422,
                message: `Related organization ${relatedParty.id} cannot be resolved locally or in the federation target`
            }
        }
...
const normalizeRelatedParty = async function(currentUser, relatedParty, tmforumEndpoint) {
    let name = ''
    ...
    if (relatedParty.id === currentUser.partyId) {
        ...
        try {
            ...
            if (tmforumEndpoint && referredType === 'Organization' && isNotFoundError(err)) {
                return await normalizeRemoteOrganizationById(relatedParty, tmforumEndpoint)
            }
    ...
    if (tmforumEndpoint && referredType === 'Organization') {
        const remoteParty = await findPartyInFederatedEndpoint(tmforumEndpoint, relatedParty.id, name)
...
const attachSellerOperator = async function(req, body, tmforumEndpoint, operatorSource) {
    if (tmforumEndpoint) {
...
const attachBuyerOperator = async function(req, body, tmforumEndpoint, operatorSource) {
    if (tmforumEndpoint) {
        const source = operatorSource || {
            id: req.user.partyId,
            href: req.user.partyId,
...
const attachProductOrderParty = async function(req, body, tmforumEndpoint) {
    ...
    if (seller) {
        body.relatedParty.push(await normalizeRelatedParty(req.user, seller, tmforumEndpoint))
        await attachSellerOperator(req, body, tmforumEndpoint, seller)
    }

    if (buyer) {
        body.relatedParty.push(await normalizeRelatedParty(req.user, buyer, tmforumEndpoint))
        await attachBuyerOperator(req, body, tmforumEndpoint, seller)
    }
}
...
exports.attachRelatedParty = async function(req, api) {
    ...
    let reqPath = req.apiUrl.split('?')[0]
    if (reqPath.endsWith('/')) {

...
const findPartyInFederatedEndpoint = async function(federationContext, credentialId) {
    ...
    const query = `externalReference.name=${encodeURIComponent(credentialId)}`
    const partyResponse = await partyClient.getOrganizationsByQueryInApi(federationContext.tmforumEndpoint, query)
    const foundParties = partyResponse && Array.isArray(partyResponse.body) ? partyResponse.body : []
    if (foundParties.length !== 1) {
        throw {
            status: 422,
            message: `Cannot resolve unique federated organization for externalReference.name=${credentialId}. matches=${foundParties.length}`
        }
    }

    const foundParty = foundParties[0]
...
const normalizeRelatedParty = async function(currentUser, relatedParty, federationContext) {
    let name = ''
    ...
    if (federationContext.tmforumEndpoint && referredType === 'Organization') {
        const remoteParty = await findPartyInFederatedEndpoint(federationContext, name)
...
const attachSellerOperator = async function(req, body, federationContext, operatorSource) {
    if (federationContext.tmforumEndpoint) {
...
const attachBuyerOperator = async function(req, body, federationContext, operatorSource) {
    if (federationContext.tmforumEndpoint) {
        const source = operatorSource || {
            id: req.user.partyId,
            href: req.user.partyId,
...
const attachProductOrderParty = async function(req, body, federationContext) {
    ...
    if (seller) {
        body.relatedParty.push(await normalizeRelatedParty(req.user, seller, federationContext))
        await attachSellerOperator(req, body, federationContext, seller)
    }

    if (buyer) {
        body.relatedParty.push(await normalizeRelatedParty(req.user, buyer, federationContext))
        await attachBuyerOperator(req, body, federationContext, seller)
    }
}

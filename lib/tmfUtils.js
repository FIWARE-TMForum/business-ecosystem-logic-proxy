...
const buildFederationContext = async function(req, partyId) {
    const context = {
        tmforumEndpoint: '',
        remotePartyByKey: {}
    }
    ...
    if (typeof federation.resolveTmforumEndpointByPartyId === 'function') {
        context.tmforumEndpoint = await federation.resolveTmforumEndpointByPartyId(sourcePartyId) || ''
    } else {
        context.tmforumEndpoint = await federation.resolveTmforumEndpoint(req) || ''
    }

    if (!context.tmforumEndpoint) {
        throw {
            status: 422,
            message: `Cannot resolve TMForum endpoint for organization ${sourcePartyId}`
        }
    }
...
const findPartyInFederatedEndpoint = async function(federationContext, localPartyId, credentialId) {
    if (!federationContext.tmforumEndpoint) {
        throw {
            status: 422,
    ...
    if (cacheKey in federationContext.remotePartyByKey) {
        return federationContext.remotePartyByKey[cacheKey]
    }

    if (typeof federation.resolveFederatedOrganizationParty !== 'function') {
        throw {
            status: 422,
            message: 'Federation organization resolver is not available'
        }
    }

    const foundParty = await federation.resolveFederatedOrganizationParty(
        federationContext.tmforumEndpoint,
        localPartyId,
        credentialId
    )
    federationContext.remotePartyByKey[cacheKey] = foundParty
    return foundParty
}

const findFederatedPartyByLocalId = async function(localPartyId) {
    if (typeof federation.resolveRemotePartyIdByLocalPartyId !== 'function') {
        throw {
            status: 422,
            message: 'Federation remote party id resolver is not available'
        }
    }
...
const normalizeRelatedParty = async function(currentUser, relatedParty, federationContext, options) {
    ...
    if (relatedParty.id === currentUser.partyId) {
        ...
        if (!name) {
            throw {
                status: 422,
                message: `Missing current user external reference for ${referredType.toLowerCase()} ${relatedParty.id}`
            }
        }
    } else {
        let party
        try {
            ...
            party = partyResp.body
        } catch (_) {
            throw {
                status: 422,
                message: `Cannot resolve ${referredType.toLowerCase()} party ${relatedParty.id}`
            }
        }

        name = getIdmExternalReferenceName(party)
        if (!name) {
            throw {
                status: 422,
    ...
    if (federationContext.tmforumEndpoint && referredType === 'Organization') {
        const remoteParty = normalizationOptions.useContextEndpointResolution
            ? await findPartyInFederatedEndpoint(federationContext, relatedParty.id, name)
            : await findFederatedPartyByLocalId(relatedParty.id)
...
const attachSellerOperator = async function(req, body, federationContext, operatorSource, normalizationOptions) {
    if (federationContext.tmforumEndpoint) {
        ...
        }, federationContext, normalizationOptions))
        return
    }
...
const attachBuyerOperator = async function(req, body, federationContext, operatorSource, normalizationOptions) {
    if (federationContext.tmforumEndpoint) {
        const source = operatorSource || {
            id: req.user.partyId,
            href: req.user.partyId,
...
const attachPartySpec = async function(req, body, federationContext) {
    ...
    if (seller) {
        body.relatedParty.push(await normalizeRelatedParty(req.user, seller, federationContext))
    ...
    await attachSellerOperator(req, body, federationContext)
}


const attachOfferingParty = async function(req, body, federationContext) {
    ...
    if ("relatedParty" in body && body.relatedParty.length > 0) {
        ...
        if (buyer) {
            parties.relatedParty.push(await normalizeRelatedParty(req.user, buyer, federationContext))
        }
    }
    ...
    // FIXME: The API is not storing two relatedParties
    await attachSellerOperator(req, body, federationContext)
}

const attachCategoryParty = async function(req, body, federationContext) {
...
const attachProductOrderParty = async function(req, body, federationContext) {
    const orderNormalizationOptions = {
        useContextEndpointResolution: true
    }
    ...
    if (seller) {
        body.relatedParty.push(await normalizeRelatedParty(req.user, seller, federationContext, orderNormalizationOptions))
        await attachSellerOperator(req, body, federationContext, seller, orderNormalizationOptions)
    }

    if (buyer) {
        body.relatedParty.push(await normalizeRelatedParty(req.user, buyer, federationContext, orderNormalizationOptions))
        await attachBuyerOperator(req, body, federationContext, seller, orderNormalizationOptions)
    }
}
...
exports.attachRelatedParty = async function(req, api) {
    ...
    const apis = {
        catalog: {
            ...
            productOfferingPrice: attachOfferingParty,
    ...
    const federationSourceParty = api === 'ordering' && objType === 'productOrder' && Array.isArray(body.relatedParty)
        ...
        : null
    const federationContext = await buildFederationContext(
        req,
        federationSourceParty && federationSourceParty.id ? federationSourceParty.id : null
    )
    await apis[api][objType](req, body, federationContext)

...
var filterRelatedParty = function (req, callback) {
    ...
    var currentUser = (req.headers['x-organization']) ? req.headers['x-organization'] : req.user.id;
    if (req.query['relatedParty.href']) {
        ...
        if(req.query['relatedParty.id'] === currentUser) {
        ...
        req.apiUrl += separator + 'relatedParty.id=' + currentUser;
        callback(null);
    }
};
...
exports.hasPartyRole = function (req, partyList, roleName) {
    var self = this;
    var user = req.headers['x-organization'] ? req.headers['x-organization'] : req.user.id;

    return req.user != null && partyList.some(function (party) {
        // FIXME: Maybe only the pathname should be compared!!
        var expectedHref = self.getIndividualURL(req, user);

        return party.id === user && party.href === expectedHref &&
...
exports.getIndividualURL = function(req, user) {
    var path = (req.headers['x-organization']) ? 'organization/' : 'individual/';
    return utils.getAPIURL(req.secure, req.hostname, config.port, '/' + config.endpoints.party.path +
			   '/api/partyManagement/v2/' + path + (user ? user : ''));
};

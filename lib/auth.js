...
function authMiddleware(idps) {
	...
	const buildOrganization = async function(element, finalRoles) {
		...
		let res = null;
		try {
			res = await party.getOrganization(element.id)
			concatOrgRoles(res)
		} catch (err) {
			if (err.status == '404') {
				const content = {
	...
	const createParty = async function(req) {
		let partyObj = null;
		try {
			partyObj = await party.getIndividual(req.user.id);
		} catch (err) {
			if (err.status == 404) {
				const user = {
					birthDate: '',
				...
				partyObj = await party.createIndividual(user)
			}
		}
		...
		if (!req.user) {
			...
			try {
				...
				if (profile.partyId == null) {
					const partyObj = await createParty(req)
					console.log('Party Created')
					console.log(partyObj)

					profile.partyId = JSON.parse(partyObj.body).id
				}
				...
				req.user.organizations.forEach(async (element) => {
					await buildOrganization(element, finalRoles)
				});

				// TODO: Check the org roles as soon as it is supported
				// await party.updateIndividual(profile.partyId, finalRoles);
				console.log('Individual updated')
			} catch (err) {

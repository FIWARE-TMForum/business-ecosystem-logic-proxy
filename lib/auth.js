...
var auth = (function () {
	...
	organizations = party.getOrganizations((err, res) => {
	    if (err) {
		utils.log(logger, 'warn', req, err.message)
		next();
	    } else {
	...
	req.user.organizations.forEach((element, index, array) => {
	    existsInd = organizations.findIndex((el, ind, arr) => { element.id === el.id });
	    if (existsInd === -1) {
		party.createOrganization(element, (err, res) => {
		    if (err) {
			utils.log(logger, 'warn', req, err.message)
		    }
		});
	    } else {
		// TODO. Check role and, in case of not being updated, update it.
		if (req.user.organizations[existsInd].id !== ) {

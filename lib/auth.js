...
var auth = (function () {
	...
	if (!req.user) {
		...
		req.user.organizations.forEach((element, index, array) => {
		    existsInd = organizations.findIndex((el, ind, arr) => {
			...
			party.getIndividual(req.user.id, (err, response) => {
			    if (err) {
				utils.log(logger, 'warn', req, err.message)
			    } else {
				console.log('user')
				console.log(JSON.parse(response.body))
				console.log('Usuario pre')
				console.log(req.user)
				// TODO: check if req.user.organizations[i].roles !== response.body.relatedParty.find[i].roles
				// and if they are, update individual.relatedParty[i]
				// if (organizations[existsInd] !== ) {

...
var auth = (function () {
	...
	if (!req.user) {
	    ...
	    var orgId = req.headers['x-organization'] ? req.headers['x-organization'] : '';
	    var org = req.user.organizations ? req.user.organizations.find( x => x.id === orgId) : undefined;
	    var orgTemplate = {};
	    if (!org && orgId != ''){
		utils.sendUnauthorized(res, 'Not allowed to access the provided organization');
	    }
	    
	    if (org) {
		// Im building the object rssClient expects
		orgTemplate.id = org.id;
		orgTemplate.roles = org.roles;
		orgTemplate.displayName = org.name;
		orgTemplate.accessToken = req.user.accesToken;
		orgTemplate.refreshToken = req.user.refreshToken;
		orgTemplate.email = org.id + '@emailnotusable.com';
	    }    
	    req.user = (req.headers['x-organization'] && orgTemplate.id) ? orgTemplate : req.user;
	...
	if (!req.user || tokensCache[req.user.accessToken].orgState != orgsEnum.PENDING){
		...
		function(callback){
			...
			party.getOrganization(element.id, (err, res) => {
				...
				callback({status: err.status, message: err.message})

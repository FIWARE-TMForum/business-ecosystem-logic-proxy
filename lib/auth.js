	...
	if (res.body.length == 0) {
		...
		if (!!element.country) {
			content.partyCharacteristic = [{
				name: "country",
				value: utils.normalizeCountry(element.country)
			}]
		}
...
function authMiddleware(idps) {
	...
	const setPartyObj = function(req, res, next) {
		const resolveOrganizationRemotePartyId = async function(partyId) {
			if (!nodeConfig.federationEnabled) {
				return '';
			}

			try {
				const remotePartyId = await federation.resolveRemotePartyIdByLocalPartyId(partyId);
				return remotePartyId || '';
			} catch (err) {
				const message = err && err.message ? err.message : JSON.stringify(err);
				logger['debug']('%s: %s', partyId || '', `Unable to resolve remotePartyId: ${message}`);
				return '';
			}
		}
		...
		if (!req.user) {
			...
			if (!org && orgId != '') {
				...
				if (org) {
					...
					orgTemplate.userId = req.user.id;
				...
				resolveOrganizationRemotePartyId(orgTemplate.partyId).then((remotePartyId) => {
					orgTemplate.remotePartyId = remotePartyId;
					req.user = orgTemplate;
					next();
				}).catch(() => {
					req.user = orgTemplate;
					next();

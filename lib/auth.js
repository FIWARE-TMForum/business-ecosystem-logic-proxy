...
const TokenService = require('../db/schemas/tokenService');
const utils = require('./utils');

// Load IDP providers
let buildStrategy;
let getScope;
if (config.oauth2.provider == 'fiware') {
    buildStrategy = require('./strategies/fiware').buildStrategy;
    getScope = require('./strategies/fiware').getScope;
} else if (config.oauth2.provider == 'keycloak') {
    buildStrategy = require('./strategies/keycloak').buildStrategy;
    getScope = require('./strategies/keycloak').getScope;
} else if (config.oauth2.provider == 'i4trust') {
    buildStrategy = require('./strategies/i4trust').buildStrategy;
    getScope = require('./strategies/i4trust').getScope;
} else if (config.oauth2.provider == 'github') {
    buildStrategy = require('./strategies/github').buildStrategy;
    getScope = require('./strategies/github').getScope;
}
...
function auth () {
    ...
    STRATEGY.userProfile = function(authToken, callback) {
        ...
        if (tokensCache[authToken] && (tokensCache[authToken].expire - moment().unix() >= 5000)) {
                ...
                if (err) {
                    ...
                    if (!userProfile.expire) {
                        ...
                        if (userProfile._json && userProfile._json.exp) {
                            ...
                            userProfile.expire = moment().unix() + 1800000;
                        }
                    }
    ...
    var headerAuthentication = function(req, res, next) {
        var askUserToken = function (token, end) {
            STRATEGY.userProfile(token, (err, userProfile) => {
                if (err) {
    ...
    var refreshToken = function refreshToken(id, refreshToken, cb) {
        STRATEGY._oauth2.getOAuthAccessToken(refreshToken, { grant_type: "refresh_token" }, (err, authToken, newRefresh) => {
            if (err) {
                cb(err);
            } else {
                let expire = moment().unix() + 3600000;
                TokenService.updateOne(
                    { userId: id },
                    { authToken: authToken, refreshToken: newRefresh, expire: expire },
                    () => {
    ...
    var refresh = function refresh(data, cb) {
        refreshToken(data.userId, data.refreshToken, (err, authToken) => {
            if (err) {
                ...
                STRATEGY.userProfile(authToken, (err) => cb(err, authToken));
            }
        });
    };
    ...
    var askProfileOrRefresh = function askProfileOrRefresh(data, cb) {
        if (data.expire - Date.now() <= 5000) {
            ...
            STRATEGY.userProfile(data.authToken, (err) => {

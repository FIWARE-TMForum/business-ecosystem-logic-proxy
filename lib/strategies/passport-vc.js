...
class CredentialSubject {
    constructor(credSubj, issuer) {
        ...
        const roleGroups = Array.isArray(credSubj.roles) ? credSubj.roles : [];
        const roles = roleGroups.length > 0 ? roleGroups[0] : { names: [] };
        const roleNames = Array.isArray(roles.names) ? roles.names : [];

        this.roles = roleNames.map(role => {
            return {
                id: role,
                name: role
            }
        ...
        this.organization = {
            id: issuer,
            name: issuer,
            roles: [roleConfig.seller, roleConfig.customer, roleConfig.orgAdmin].map(role => ({
                'name': role,
                'id': role
            }))
        };
...
class LEARCredSubject {
    constructor(credSubj, issuer) {
        ...
        this.id = individual.id || credSubj.id
        this.email = individual.email
        this.familyName = individual.last_name || individual.lastName
        this.firstName = individual.first_name || individual.firstName
        this.roles = []
    ...
    mapRoles(mandate) {
        ...
        return mandate.power.map((power) => {
            ...
            if (func == 'onboarding' && hasPower(action, 'execute')){
                // orgAdmin
                role = {
                    'id': roleConfig.orgAdmin,
                    'name': roleConfig.orgAdmin
                }
            } else if ('productoffering' && hasPower(action, 'create') && hasPower(action, 'update')) {
                // Seller
                role = {
                    'id': roleConfig.seller,
                    'name': roleConfig.seller
                }
                ...
                role = {
                    'id': roleConfig.certifier,
                    'name': roleConfig.certifier
                }
            }
...
const getLearCredentialSubject = (type) => {
    let subject = null;

    credentialTypes(type).forEach((credType) => {
        ...
        if (credType == 'LEARCredentialEmployee' || normalizedType.startsWith('learcredential.employee.w3c.')) {
            subject = LEARCredentialSubject;
        } else if (credType == 'LEARCredentialMachine' || normalizedType.startsWith('learcredential.machine.w3c.')) {
            subject = LEARCredentialMachineSubject;
        }
    });
...
class VerifiableCredential {
    constructor(payload) {
        const processSubject = (data) => {
            ...
            let subject = CredentialSubject;  // Default credential format from FIWARE Issuer
            const learSubject = getLearCredentialSubject(data.type);

            // Check if we have explicit VC format defined (DOME)
            if (learSubject != null) {
                subject = learSubject;
            }
        ...
        if (payload.hasOwnProperty('credentialSubject')) {
            ...
            const presentations = payload['verifiablePresentation'];
            const data = presentations.find((pres) => {
                return pres.type != null && getLearCredentialSubject(pres.type) != null;
            })
            processSubject(data || presentations[0])
        } else {
            throw new Error('VC not found');
        }
    }
    ...
    getProfile() {
        const profile = {};
        // Use credential subject identifier when available, fallback to token subject.
        profile.id = this.credentialSubject.id || this.payload.sub;
...
class VCStrategy extends Strategy {
    constructor(options, verify) {
        ...
        this.signAlgorithm = options.signAlgorithm;
    ...
    buildClientAssertion() {
        ...
        const token = jwtSigner.signJwt(tokenInfo, {
            keyid: this.clientID,

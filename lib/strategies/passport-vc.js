    ...
    userProfile(accessToken, done) {
        try {
            ...
            if (payload['verifiableCredential']) {
                profile.email = payload['verifiableCredential']['credentialSubject']['email'];
                profile.displayName = 'display name'
                profile.id = payload['verifiableCredential']['credentialSubject']['id'];
                profile.roles = payload['verifiableCredential']['credentialSubject']['roles'].map(role => {
                    return {
                        id: role['names'][0],
                        name: role['names'][0]
                    }

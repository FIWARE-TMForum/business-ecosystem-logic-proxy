...
class LEARCredentialSubject extends LEARCredSubject {
    ...
    mapRoles(mandate) {
        return mandate.power.map((power) => {
            ...
            if ((!v2 && func == 'onboarding' && action.toLowerCase() == 'execute') ||
             (v2 && func =='onboarding' && action.includes('execute'))){
                // orgAdmin
                role = {
                    'id': 'orgAdmin',
                    'name': 'orgAdmin'
                }
            } else if ((!v2 && func == 'productoffering' && action.includes('Create') && action.includes('Update')) || 
                    (v2 && func == 'productoffering' && action.includes('create') && action.includes('update'))) {
                // Seller
                role = {
                    'id': 'seller',
                    'name': 'seller'
                }
            } else if ((!v2 && func == 'certification' && (action.includes('Upload') || action.includes('post_verifiable_certification'))) ||
                        (v2 && func == 'certification' && (action.includes('uploadCertificate') || action.includes('post_verifiable_certification')))) {
                role = {
                    'id': 'certifier',
                    'name': 'certifier'
                }

...
function strategy (config) {
    ...
    async function buildStrategy(callback) {
        ...
        return new KeyCloakStrategy(params, (accessToken, refreshToken, profile, done) => {
            ...
            if (rawProfile.resource_access != null && rawProfile.resource_access.bae != null) {
                profile.roles = rawProfile.resource_access.bae.roles.map((role) => {
                    return {
                        id: role,
                        name: role
                    }
            ...
            if (rawProfile.groups != null) {
                profile.organizations = rawProfile.groups.map((group) => {
                    return {
                        id: group,
                        name: group,
                        roles: [{

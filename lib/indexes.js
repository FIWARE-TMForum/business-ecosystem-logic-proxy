...
// var logger = require('./logger').logger.getLogger('TMF');
...
exports.saveIndexInventory = function saveIndexInventory(data) {
    ...
    data.forEach(function(invData) {
        promise = promise.then(() => {
            return addOfferInformationToInventory(invData).then((offerData) =>
...
var createUrl = function createUrl(api, extra) {
    return utils.getAPIProtocol(api) + '://' + utils.getAPIHost(api) + ':' + utils.getAPIPort(api) + '/' + api + extra;
};

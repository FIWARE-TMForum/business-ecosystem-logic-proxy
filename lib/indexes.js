...
exports.convertCatalogData = function convertCatalogData(data) {
    var initialData = { id: "catalog:" + data.id,
                        originalId: data.id,
                        sortedId: leftPad(data.id, 12, 0),
                        relatedPartyHash: data.relatedParty ? data.relatedParty.map(x => exports.fixUserId(x.id)) : '',
                        relatedParty: data.relatedParty ? data.relatedParty.map(x => x.id) : ''
                      };
...
var convertOffer = function convertOffer(data, user) {
    ...
    if (!!data.category && data.category.length > 0) {
        return blockPromises(data.category, cat => {
            ...
            return indexCreateData(["href", "name", "lifecycleStatus", "isBundle", "catalog"], data, initialData);
        });
    }

    var newData = indexCreateData(["href", "name", "lifecycleStatus", "isBundle", "catalog"], data, initialData);

...
// SEARCH
var createQuery = function createQuery(query, offset, size, sort) {
    var q = {};
    if (query)
        q.query = query;
    if (offset)
...
exports.searchUserId = function searchUserId(f, id, offset, size, sort) {
    var q = createQuery({ AND: { userId: [exports.fixUserId(id)] } }, offset, size, sort);
    return f(q);
};
    ...
    data.forEach(function(invData) {
        promise = promise.then(() => {
            addOfferInformationToInventory(invData)
                .then((offerData) => {
                    saveIndex('inventory', [convertInventoryData(offerData)], opts)
                });
        });
    });

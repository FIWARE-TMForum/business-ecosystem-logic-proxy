...
var queryIndexExecute = function queryIndexExecute(path, method) {
    ...
    lock.acquire("searchIndex", () => p, function () {
...
exports.searchOfferings = exports.search.bind(null, 'offerings');

exports.searchProducts = exports.search.bind(null, 'products');

exports.searchCatalogs = exports.search.bind(null, 'catalogs');

exports.searchInventory = exports.search.bind(null, 'inventory');

exports.searchOrders = exports.search.bind(null, 'orders');
...
var getUserFromOffer = function getUserFromOffer(id, def) {
    ...
    return exports.searchProductId(id)
        .then(results => (results.length > 0 && results[0].document.relatedParty.length > 0) ? { id: results[0].document.relatedParty[0] } : {})
...
function addOfferInformationToInventory(data) {
    var p = new Promise();

    // Will set the name and description of the product offering instead
    var url = createUrl(data.productOffering.href.split('/').slice(3)[0], "/" + data.productOffering.href.split('/').slice(4).join('/'));

    request(url, function (err, response, body) {
        if (err) {
            p.reject(err);
        } else {
            var resp = JSON.parse(response.toJSON().body);
            var description = typeof resp.description !== 'undefined' ? resp.description.toLowerCase() : '';

            data.searchable = [resp.name.toLowerCase(), description];
            p.resolve(data);
        }
    });
...
exports.saveIndexInventory = function saveIndexInventory(data) {
    ...
    data.forEach(function(invData) {
        promise = promise.then(() => {
            addOfferInformationToInventory(invData)
                .then((offerData) => {
                    exports.saveIndex('inventory', [exports.convertInventoryData(offerData)], opts)
                });
        });
    });
...
exports.saveIndexCatalog = function saveIndexCatalog(data) {
    ...
    return exports.saveIndex('catalogs', newData, ops);
};
...
exports.saveIndexProduct = function saveIndexProduct(data) {
    ...
    return exports.saveIndex('products', newData, ops);
};
...
exports.saveIndexOffering = function saveIndexOffering(data, user) {
    ...
    return blockConvert(notBundle, user)
        .then(newData => exports.saveIndex('offerings', newData, ops))
...
exports.saveIndexOrder = function saveIndexOrder(data) {
    ...
    return exports.saveIndex('orders', newData, opts);
};

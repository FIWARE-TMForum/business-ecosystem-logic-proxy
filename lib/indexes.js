...
function addOfferInformationToInventory(data) {
    ...
    request(url, function (err, response, body) {
        if (err) {
            ...
            data.searchable = [JSON.parse(response.toJSON().body).name.toLowerCase(), JSON.parse(response.toJSON().body).description.toLowerCase()];
            p.resolve(data);
        }
    });
...
exports.saveIndexInventory = function saveIndexInventory(data) {
    // Adds the searchable propertie to data
    var updatedData = data.map(addOfferInformationToInventory);
    // Waits for all promises to resolve and then keeps synchronously
    return Promise.all(updatedData)
        .then(updatedData => updatedData.map(exports.convertInventoryData))
        .then(newData => exports.saveIndex(exports.siTables.inventory, newData, {})); // ops = {}
};

exports.convertCatalogData = function convertCatalogData(data) {
    var searchable = [data.name.toLowerCase(), data.description.toLowerCase()];
...
exports.convertProductData = function convertProductData(data) {
    var searchable = [data.name.toLowerCase(), data.brand.toLowerCase(), data.description.toLowerCase()];
...
var convertOffer = function convertOffer(data, user) {
    ...
    if (!!data.category && data.category.length > 0) {
        return blockPromises(data.category, cat => {
            ...
            initialData.categoriesId = categories.map(x => "cat:" + x.id);
...
exports.saveIndexOffering = function saveIndexOffering(data, user) {
    ...
    return blockConvert(notBundle, user)
        .then(newData => exports.saveIndex(exports.siTables.offerings, newData, ops))

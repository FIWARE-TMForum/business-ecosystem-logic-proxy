...
exports.getMiddleware = function getMiddleware(reg, createOfferF, searchF, keysUsed, req) {
    if (req.method == "GET" && reg.test(req.apiUrl) && (typeof req.query.id === "undefined")) {
        ...
        return searchF(q)
            .then(results => {
                var newUrl = req._parsedUrl.pathname;
                if (results.hits.length > 0) {
                    newUrl = newUrl + "?id=" + results.hits.map(r => r.document.originalId).join(",");
                }

                var separator = "?";

                for (var key in req.query) {
                    separator = (separator === "&" || newUrl.indexOf("?") >= 0) ? "&" : "?";

                    if (keysUsed.indexOf(key) < 0) {
                        newUrl = newUrl + separator + key + "=" + req.query[key];
                    }
                }

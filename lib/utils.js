...
exports.attachUserHeaders = function(headers, userInfo) {
    ...
    var roles = [
        { roleName: config.oauth2.roles.admin, role: 'provider'},
        { roleName: config.oauth2.roles.seller, role: 'seller' },
        { roleName: config.oauth2.roles.customer, role: 'customer'}
    ];

    roles.forEach(function (role) {
        if (exports.hasRole(userInfo, role.roleName)) {
            headers['X-Roles'] += role.role + ',';
        }
    });
...
exports.hasRole = function (user, roleName) {
    return roleName == '__any__' || (user != null && 'roles' in user && user.roles.some(function (role) {
...
/**
 * Build OR conditions for index queries using coma-separated strings
 * @param {String} osString
 * @param {String} name
 * @param {Object} query
 * @param {Function} filter
 */
exports.queryAndOrCommas = function queryAndOrCommas(orString, name, query, filter) {
    if (!filter) {
    ...
    if (orString.split(",").length <= 1) {
        indexes.addAndCondition(query, { [name]: [filter(orString)] });
    } else {
        indexes.addOrCondition(query, name, orString.split(",").map(filter));
    }
};

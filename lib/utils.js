...
exports.attachUserHeaders = function(headers, userInfo) {
    ...
    headers['X-Roles'] = '';
    headers['X-Organizations'] = '';

    var parseOrgs = function (org) {
	headers['X-Organizations'] += new Buffer(JSON.stringify(org.id)).toString("base64") + ',';
    }
    ...
    // I have tested this: This works and... well, maybe its not the best readable way of
    // parsing an array of json objects, but i think its the best way of doing it.
    // also, im not sending the whole organizations array, parsing just an array of id's.
    // want some more info about the orgs? Request them.
    if (userInfo.organizations){
	userInfo.organizations.forEach(parseOrgs)
    }
    //TODO: Check what info about the currentUser is being sent on the request

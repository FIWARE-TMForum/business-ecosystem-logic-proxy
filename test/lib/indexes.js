...
describe('Test index helper library', function() {
        ...
        testIndexInit(
            ...
            (levelMock, searchIndex) => {
                ...
                expInd.forEach((indexPath) => {
                    ...
                    expect(levelMock).toHaveBeenCalledWith(
                        indexPath,
                        {
                            valueEncoding: 'json',
                            db: down
    ...
    var helper = function helper(extra, f1, success, error) {
        ...
        indexes[f1]
            .apply(this, Array.prototype.slice.call(arguments, 4))
            ...
            () => {
                ...
                auxIds.forEach((id) => {
                    var curl =
                        ...
                        '/' +
                        catInfo.path +
                        '/api/catalogManagement/v2/category/' +
                        id;
    ...
    it('should save converted offering with multiple categories', function(done) {
        testSaveCategoryOffering(
            [13, 14],
            notBundleMultipleCategoriesOffer,
            notBundleMultipleCategoriesOfferExpected,
            done
        );
    });
        ...
        helper(
            ...
            (si, err) => {
                ...
                var curl =
                    ...
                    '/' +
                    catInfo.path +
                    '/api/catalogManagement/v2/category/13';
    ...
    it('should convert offer with bundle searching user in products and offerings index', function(done) {
        ...
        testSaveIndexes(
            ...
            (si) => {
                [
                    { query: { AND: { sortedId: ['000000000001'] } } },
                    { query: { AND: { sortedId: ['000000000002'] } } }
                ].forEach((q) => {
                    expect(si.search).toHaveBeenCalledWith(q);
                });
    ...
    it('should save converted inventory data correctly', function(done) {
        ...
        testSaveIndexes(
            ...
            (si) => {
                [
                    { query: { AND: { sortedId: ['000000000005'] } } },
                    { query: { AND: { sortedId: ['000000000006'] } } }
                ].forEach((q) => {
                    expect(si.search).toHaveBeenCalledWith(q);
                });
                done();
    ...
    describe('Request helpers', function() {
        ...
        var testSearchMiddlewareQuery = function(req, search, done) {
            ...
            indexes.getMiddleware(fs.reg, fs.createOffer, fs.search, req).then(() => {
                expect(fs.reg.test).toHaveBeenCalledWith('url');
        ...
        it('should execute middleware and return count URL when action param included', function(done) {
            testSearchMiddleware(
                ...
                '/' + testUtils.getDefaultConfig().endpoints.management.path + '/count/0',
                true,
                0,
                done
            );
